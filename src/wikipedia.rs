...
pub fn hamming_encode(n: u8) -> u8 {
    // Data bits
    let d: [u8; 4] = [n & 1, n >> 1 & 1, n >> 2 & 1, n >> 3 & 1];
...
fn hamming_error_correct(x: u8) -> (u8, ErrorType) {
    ...
    let syndrome: usize = (s[2] << 2 | s[1] << 1 | s[0]) as usize;
    ...
    if x & 1 != x_parity {
        if syndrome > 0 {
            error_type = ErrorType::SingleBit;
        } else {
            return (x ^ 1, ErrorType::ParityBit);
        }
    } else if syndrome == 0 {
        return (x, ErrorType::None);
    } else {
        error_type = ErrorType::DoubleBit;
    }
...
pub fn hamming_decode(x: u8) -> (u8, ErrorType) {
    ...
    (d[3] << 3 | d[2] << 2 | d[1] << 1 | d[0], error_type)
}
        ...
        for encoded in x {
            for i in 0..NUM_BITS - 1 {
                for j in i + 1..NUM_BITS {
                    ...
                    assert_eq!(error_type, ErrorType::DoubleBit);
                }
            }
        }
    }
    ...
    fn test_single_bit_or_parity_error() {
        ...
        for (i, encoded) in x.iter().enumerate() {
            for bit in 0..8 {
                ...
                assert_eq!(decoded, N[i]);
                if bit == 0 {
                    assert_eq!(error_type, ErrorType::ParityBit);
                } else {
                    assert_eq!(error_type, ErrorType::SingleBit);
                }
            }
        }
    }
